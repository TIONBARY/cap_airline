...
- **출제는 난이도 등급(`diff` 1~5) 확률로 정해진다** (`pickCountry`). 첫 문제는
  `38/27/20/10/5%`로 시작해 마지막엔 `10/15/20/25/30%`로 뒤집힌다 → 뒤로 갈수록 어려워진다.
  비복원이라 **중복 없음·전 국가 출제** 성질은 그대로다.
- **첫 3문제는 1~2등급만 나온다** (`WARMUP`). 확률이 아니라 **확정 보장**이다.
  4번째 문제부터 위 확률로 복귀한다. 이유는 아래 "문제 출제 알고리즘" 참고.
- 비행기가 나라에 도착하면 그 자리(핀)에 **국기 + 나라명**이 뜬다. 하단 바에 수도를 입력.
- **제한시간 `ANSWER_TIME` (현재 7초).** 도착하는 순간 입력창 위 게이지가 줄어들기 시작하고,
...
- `COUNTRIES[]` — `{ code, country, capital, lat, lon, pop, diff }` **100개국**.
  ...
  - **`diff`** — 난이도 **1~5** (1=상식, 5=거의 모름). **출제 확률을 정하는 유일한 값**이다.
    ...
    - ⚠️ **주관적 추정치다.** 정확히 하려면 `scores`에 오답 기록을 남겨 나라별 정답률로 실측해야 한다
      (아래 "향후 아이디어" 참고)
    - ⚠️ **등급이 권역과 강하게 겹쳐 있다** — 아프리카 33개국 중 28개(85%)가 4~5등급이고,
      4~5등급 41개국 중 아프리카가 28개(68%)다. 그래서 "어려운 등급을 뽑는다"는 게
      체감상 "아프리카를 뽑는다"와 거의 같다. 등급을 조정할 땐 이 편중을 염두에 둘 것
- `WORLD_GRID` — 120×60 블럭 세계지도 (문자: `.`바다 `g`초원 `d`사막 `i`빙하).
  실제 위성지도를 다운샘플링해 만든 것. 대륙 모양 인식 가능.

### 문제 출제 알고리즘 (`buildDeck` + `pickCountry`)

**만족해야 하는 제약 4가지** — 하나라도 깨면 다른 기능이 망가진다.
1. **한 바퀴(100문제) 안에 중복 없음**
2. **한 바퀴에 100개국이 전부 등장** — 안 그러면 "세계일주 완주" 연출이 거짓말이 된다
3. **난이도가 뒤로 갈수록 올라간다**
4. **첫 3문제에는 3등급 이상이 안 나온다** (`WARMUP`) — 손도 못 써보고 끝나는 판 방지
...
```js
const TIER_W_START = { 1: 38, 2: 27, 3: 20, 4: 10, 5:  5 };   // 첫 문제 기준 등급별 확률(%)
const TIER_W_END   = { 1: 10, 2: 15, 3: 20, 4: 25, 5: 30 };   // 마지막 문제 기준
// 진행도 p = 1 - deck.length/100 으로 START → END 선형 보간
```

**추가 장치: 워밍업 (`WARMUP = 3`, `WARMUP_MAX_DIFF = 2`)**

첫 3문제는 **1~2등급에서만** 뽑는다. 확률이 아니라 확정이다.

- **왜 확률 완화만으로 부족한가** — 난이도 등급이 사실상 **권역과 겹쳐 있다.**
  아프리카 33개국 중 **28개(85%)가 4~5등급**이고, 뒤집으면 4~5등급 41개국 중
  **아프리카가 28개(68%)**다. 그래서 초반에 4~5등급이 한 번만 떠도 체감은
  "시작하자마자 또 아프리카"가 된다. 목숨이 3번뿐이라 그대로 판이 끝난다.
  확률만 낮추면 기댓값은 내려가도 **"첫 문제부터 5등급"이라는 사건 자체는 계속 남는다.**
- **구현** — 워밍업 구간에서 3등급 이상의 가중치를 0으로 주고 `left`(등급별 남은 개수)
  집계에서도 뺀다. 가중치만 건드리므로 **비복원 성질이 그대로라 제약 1·2가 안 깨진다.**
  4번째 문제부터 아무 처리 없이 원래 확률로 복귀한다.
- 워밍업 대상이 35개국(1·2등급)이라 3장을 빼도 고갈되지 않는다.

**실측 (`index.html`의 상수를 그대로 읽어 20,000판)**

| 검증 항목 | 결과 |
|---|---|
| 한 판 = 정확히 100장 | ✅ 위반 0 |
| 한 판 안에 중복 | ✅ 위반 0 |
| 직전 나라 연속 출제 | ✅ 위반 0 (`lastCode` 가중치 0) |
| 첫 3문제가 2등급 이하 | ✅ 위반 0 |
| 뒤로 갈수록 어려움 | ✅ 후반(61~100) − 초반(1~15) 평균등급 **+1.61** |
| 첫 문제 등급 분포 | **58.4 / 41.6 / 0 / 0 / 0 %** (워밍업이 3등급 이상을 막는다) |

난이도 곡선 — `1~5문제 1.74등급` → `6~15 2.30` → `16~30 2.47` → `31~60 3.01` → `61~100 3.73`
...
|---|---|---|
| 첫 문제가 4~5등급 | 25.3% | **0%** |
| 첫 3문제에 4~5등급 포함 | 58.4% | **0%** |
| 첫 5문제 중 아프리카·중동 | 1.61개 | **0.82개** |
| 2점 이하로 끝나는 판 | 15.4% | **1.3%** |

(체감 점수는 등급별 정답률을 `97/88/68/35/12%`로 가정해 3오답까지 굴린 시뮬레이션.
가정값이라 절대치는 못 믿지만 **안 사이 비교**에는 쓸 수 있다. 중앙값 6점 → 11점)

**설계 의도** — 예전엔 "첫 문제부터 5등급이 나올 수 있다"를 *운도 게임의 일부*로 의도했었는데,
실제로 측정해보니 초반 사망률이 너무 높아 게임이 시작도 못 하고 끝났다. **초반 3문제만
확정으로 막고, 그 뒤의 복불복은 그대로 남겼다.**
...
({ 제약위반: 판.filter(d=>d.length!==N||new Set(d.map(c=>c.code)).size!==N).length,
   워밍업위반: 판.filter(d=>d.slice(0,WARMUP).some(c=>c.diff>WARMUP_MAX_DIFF)).length,
...
```
> `제약위반` · `워밍업위반`이 **둘 다 0**이어야 한다. 하나라도 0이 아니면 세계일주 완주
> 연출이나 초반 보호가 깨진 것이다.

**`pickCountry()`** 는 위 가중치로 남은 풀에서 한 장을 `splice`로 빼낸다. 덱이 비면
`buildDeck()`으로 재충전하고, 이음매에서 직전 나라가 연속으로 나오는 것만 `lastCode` 가중치를
0으로 줘서 막는다. 워밍업 여부는 **`COUNTRIES.length - deck.length`(이번 판의 문제 번호)**로
판단한다 — `startGame()`이 `deck = []`로 리셋하므로 이 값이 곧 "판 시작부터 몇 번째"다.
가중치 0인 칸에 떨어지는 부동소수점 예외를 대비해 `findIndex` 안전망이 뒤에 붙어 있고,
그 안전망도 **워밍업 제한을 지킨다**(안 지키면 첫 문제에 5등급이 새어 나온다).
...
## 알려진 이슈 / 참고
- ✅ (해결됨) **초반 난이도가 너무 높던 문제** — "시작하자마자 아프리카·중동만 나온다"는 실제 플레이 피드백.
  측정해보니 첫 문제가 4~5등급일 확률 **25.3%**, 첫 3문제 안에 낄 확률 **58.4%**였고,
  4~5등급의 68%가 아프리카라 체감이 더 나빴다. → `TIER_W_START` 완화(`30/25/20/15/10` →
  `38/27/20/10/5`) + **워밍업 3문제 확정 보장**. 자세한 건 "문제 출제 알고리즘" 참고.
...
**완료된 것** — **초반 난이도 완화(워밍업 3문제 + 가중치 조정)** /
세계일주 완주 = 엔딩(만점 100점) / 난이도 등급(`diff`) 기반 출제 확률 / 거리 비례 비행 시간 /
...
- **`TIER_W_START` / `TIER_W_END`** — 난이도 등급별 출제 확률(%). 첫 문제 → 마지막 문제로
  선형 보간된다. 현재 `38/27/20/10/5` → `10/15/20/25/30`.
  - **초반이 너무 빡세면** `TIER_W_START`의 4·5등급을 더 낮춘다
  - **후반이 안 어려우면** `TIER_W_END`의 5등급을 올린다
  - 둘을 같게 만들면 고정 가중치가 되는데, **그러면 53번째 문제까지 난이도가 평평해진다**(측정 확인).
    권장하지 않는다
  - 바꿨으면 위 "문제 출제 알고리즘"의 재측정 스니펫을 돌릴 것
- **`WARMUP` / `WARMUP_MAX_DIFF`** — 판 시작 몇 문제를 몇 등급 이하로 보장할지 (현재 **3문제 / 2등급 이하**).
  **초반 이탈에 가장 직접적으로 듣는 값이다.**
  - 더 순하게 하려면 `WARMUP`을 5로 (첫 5문제 전부 보장). 대신 매 판 초반이 비슷해져 반복 플레이가 지루해진다
  - 워밍업을 아예 끄려면 `WARMUP = 0`. 그럼 `TIER_W_START` 확률만으로 돌아간다
  - `WARMUP_MAX_DIFF`를 3으로 올리면 보호 효과가 크게 떨어진다 — 3등급에도 알제·튀니스 같은 게 있다
  - ⚠️ `WARMUP`이 1·2등급 나라 수(35개)를 넘으면 안 된다. 넘으면 워밍업 후보가 고갈된다
...
**세계일주 완주를 무한 반복이 아니라 엔딩으로 (만점 100점 확정)** →
**초반 난이도 완화 (워밍업 3문제 1~2등급 보장 + 출제 가중치 조정)** 순으로 발전.
